// This script is used to plot the energy of incoming neutrinos based on interaction type and inetraction mode. 
...
for (Long64_t i = 0; i < numEntries; ++i) {
    ...
    if ( _truemcpKE >= 0.05&& nu_mode == 0 && nu_ccnc == 0)
        trueEnuStartQE_CC->Fill(nu_energy);
    if ( _truemcpKE >= 0.05&& nu_mode == 1 && nu_ccnc == 0)
        trueEnuStartRES_CC->Fill(nu_energy);
    if ( _truemcpKE >= 0.05&& nu_mode == 2 && nu_ccnc == 0)
        trueEnuStartDIS_CC->Fill(nu_energy);
    if ( _truemcpKE >= 0.05&& nu_mode == 3 && nu_ccnc == 0)
        trueEnuStartCOH_CC->Fill(nu_energy);
    if ( _truemcpKE >= 0.05&& nu_mode == 10 && nu_ccnc == 0)
        trueEnuStartMEC_CC->Fill(nu_energy);
}
...
hs_CC->Draw("hist");
hs_CC->SetTitle(" ");
hs_CC->GetXaxis()->SetTitle("Neutrino Energy (GeV)");
...
// c3.Print("EnuStart_mode_CC_noCut_cutOn50MeVke.png");
// c3.Print("EnuStart_mode_CC_noCut.png");
